...
for(i in 1:nrow(my_streams)){
  ...
  if(my_streams$city[i]=="ATL"){
    ...
    # read weather data and add to 
    # calculate solar time using site longitude
    atlanta_stream_data$solar.time <- calc_solar_time(atlanta_stream_data$dateTime, 
                                              as.numeric(my_streams$longitude[i]))
    
    # simulate light in PAR (umol m^-2 s^-1)
    atlanta_stream_data$light <- calc_light(atlanta_stream_data$solar.time, 
                                    as.numeric(my_streams$latitude[i]),
                                    as.numeric(my_streams$longitude[i]))
    
    # calculate 100% DO saturation concentration (mg/L)
    atlanta_stream_data$DO.sat <- calc_DO_sat(atlanta_stream_data$temp.water, 
                                      (atlanta_stream_data$BP_mmHg)*1.33322)
    
    # and discharge
    atlanta_hydro <- readNWISuv(my_streams$gage[i], 
                            c("00060", "00065"), 
                            tz = "EST",
                            startDate = my_streams$start[i], 
                            endDate = my_streams$end[i])

    atlanta_hydro$discharge <- atlanta_hydro$X_00060_00000/35.315
    atlanta_hydro$depth <- atlanta_hydro$X_00065_00000/3.28084
    
    atlanta_stream_data <- merge(atlanta_stream_data, atlanta_hydro[,c(3,9,10)], all.x = T)
    atlanta_stream_data <- distinct(atlanta_stream_data, solar.time, .keep_all = T)
    
    if(my_streams$SN[i]=="AB"){
      atlanta_stream_data <- atlanta_stream_data[-37614,]
    }

    if(my_streams$SN[i]=="SB"){
      atlanta_stream_data <- atlanta_stream_data[-37630,]
    }
    
    
    atlanta_stream_data$discharge <- na.approx(atlanta_stream_data$discharge)
    atlanta_stream_data$depth <- na.approx(atlanta_stream_data$depth)
...
for(i in 1:nrow(my_streams)){
  ...
  if(my_streams$city[i]=="PDX"){
    ...
    # start with USGS data
    portland_stream_data <- readNWISuv(my_streams$gage[i], 
                                       c("00060", "00065","00010","00095"), 
                                       tz = "PST",
                                       startDate = my_streams$start[i], 
                                       endDate = my_streams$end[i])
    
    # split data file into separate frames for each stream site in this city
    portland_stream_data <- portland_data[portland_data$site==my_streams$streamname[i],]
    
    # calculate solar time using site longitude
    portland_stream_data$solar.time <- calc_solar_time(portland_stream_data$dateTime, 
                                              as.numeric(my_streams$longitude[i]))
    
    # simulate light in PAR (umol m^-2 s^-1)
    portland_stream_data$light <- calc_light(portland_stream_data$solar.time, 
                                    as.numeric(my_streams$latitude[i]),
                                    as.numeric(my_streams$longitude[i]))
    
    # calculate 100% DO saturation concentration (mg/L)
    portland_stream_data$DO.sat <- calc_DO_sat(portland_stream_data$temp.water, 
                                      (portland_stream_data$BP_mmHg)*1.33322)

    portland_hydro$discharge <- portland_hydro$X_00060_00000/35.315
    portland_hydro$depth <- portland_hydro$X_00065_00000/3.28084
    
    portland_stream_data <- merge(portland_stream_data, portland_hydro[,c(3,9,10)], all.x = T)
    portland_stream_data <- distinct(portland_stream_data, solar.time, .keep_all = T)

    portland_stream_data$discharge <- na.approx(portland_stream_data$discharge)
    portland_stream_data$depth <- na.approx(portland_stream_data$depth)
